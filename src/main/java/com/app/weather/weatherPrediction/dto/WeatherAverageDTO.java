...
public class WeatherAverageDTO implements Serializable {
	...
	private BigDecimal daily_temp_min;

	private BigDecimal daily_temp_max;
	...
	@JsonIgnore
	private BigDecimal totalDailyTempMin;

	@JsonIgnore
	private BigDecimal totalDailyTempMax;
	...
	public WeatherAverageDTO() {
		...
		this.totalDailyTempMin = BigDecimal.ZERO;
		this.totalDailyTempMax = BigDecimal.ZERO;
	...
	public void plusMap(WeatherMapTimeDTO map) {
		if (map.isDaily()) {
			...
			this.totalDailyTempMax = this.totalDailyTempMax.add(map.getMain().getTemp_max());
			this.totalDailyTempMin = this.totalDailyTempMin.add(map.getMain().getTemp_min());
			this.quantDaily++;
		} else {
	...
	public void totalize() {
		this.daily = (this.quantDaily > 0)
				...
				: null;
		this.daily_temp_max = (this.quantDaily > 0)
				? this.totalDailyTempMax.divide(new BigDecimal(this.quantDaily.toString()), 2, RoundingMode.HALF_UP)
				: null;
		this.daily_temp_min = (this.quantDaily > 0)
				? this.totalDailyTempMin.divide(new BigDecimal(this.quantDaily.toString()), 2, RoundingMode.HALF_UP)
				: null;
	...
	public BigDecimal getDaily_temp_min() {
		return daily_temp_min;
	}

	public void setDaily_temp_min(BigDecimal daily_temp_min) {
		this.daily_temp_min = daily_temp_min;
	}

	public BigDecimal getDaily_temp_max() {
		return daily_temp_max;
	}

	public void setDaily_temp_max(BigDecimal daily_temp_max) {
		this.daily_temp_max = daily_temp_max;
	}

	public BigDecimal getTotalDailyTempMin() {
		return totalDailyTempMin;
	}

	public void setTotalDailyTempMin(BigDecimal totalDailyTempMin) {
		this.totalDailyTempMin = totalDailyTempMin;
	}

	public BigDecimal getTotalDailyTempMax() {
		return totalDailyTempMax;
	}

	public void setTotalDailyTempMax(BigDecimal totalDailyTempMax) {
		this.totalDailyTempMax = totalDailyTempMax;
	}
